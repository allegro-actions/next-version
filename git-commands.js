...
export function getLatestTag(prefix) {
  ...
  const stdout = execSync(`(git ls-remote --tags --refs origin | cut -d'/' -f3 && git tag -l) ${maybeGrepCommand} | sed '/-/!{s/$/_/;}; s/-patch/_patch/' | sort -V | sed 's/_$//; s/_patch/-patch/' | tail -n 1`);
