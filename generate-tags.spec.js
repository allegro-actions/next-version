const action = require('./generate-tags');
...
describe('generate-tags', () => {
  ...
  test('increments pre release version', () => {
    ...
    expect(
      action({ versioning: 'single-number', preReleaseSuffix: 'RC', level: 'prerelease' }, () => 'v1-RC.1'),
    ).toEqual({
      currentTag: 'v1-RC.1',
      nextTag: 'v1-RC.2',
      nextVersion: '1-RC.2',
  ...
  test('when there is no suffix for pre release', () => {
    // expect
    expect(() =>
      action({ versioning: 'semver', preReleaseSuffix: '', level: 'prerelease' }, () => 'v1.0.0-RC.0'),
    ).toThrowError();
    expect(() =>
      action({ versioning: 'single-number', preReleaseSuffix: '', level: 'prerelease' }, () => 'v1-RC.0'),
    ).toThrowError();
  });
  ...
  test('throws error when level is invalid', () => {
    // expect
    expect(() =>
      action({ versioning: 'semver', preReleaseSuffix: 'RC', level: 'invalid' }, () => 'v1.0.0-RC.0'),
    ).toThrowError();
    expect(() =>
      action({ versioning: 'single-number', preReleaseSuffix: 'RC', level: 'invalid' }, () => 'v1-RC.0'),
    ).toThrowError();
  });
  ...
  test('increments version with custom prefix', () => {
    ...
    ).toEqual({ currentTag: 'prefix-1', nextTag: 'prefix-2', nextVersion: '2' });
  });
